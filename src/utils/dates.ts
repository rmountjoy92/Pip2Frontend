...
class Dates {
  getDateObject(timestamp: string | number | Date): Date {
    return new Date(timestamp)
  }

  toTimestamp(dateObj: Date): string {
    return date.formatDate(dateObj, `YYYY-MM-DD HH:mm ZZ`)
  }

  formatDate({ timestamp, formatString }: FormatDateOptions): string | undefined {
    const dateObj = this.getDateObject(timestamp)
    return date.formatDate(dateObj, formatString)
  }

  relativeDate(timestamp: string | number | Date): string {
    const dateObj = this.getDateObject(timestamp)
    const now = new Date()
    ...
    if (diffMinutes < 0 && diffMinutes > -60) {
      return `${diffMinutes * -1} minute${diffMinutes !== -1 ? 's' : ''} from now`
    }
  ...
  addToDate(timestamp: string | number | Date, timeObj: TimeObject): Date {
    const dateObj = this.getDateObject(timestamp)
